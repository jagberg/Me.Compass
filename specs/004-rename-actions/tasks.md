...
- [X] T005 Add `backend/src/db/migrations/003_rename_actions.sql`:
      `ALTER TABLE action ADD COLUMN title_pinned INTEGER NOT NULL DEFAULT 0;`
- [X] T006 Add `title_pinned: boolean` to the `Action` interface in `backend/src/types/index.ts`
- [X] T007 Plumb `title_pinned` through `backend/src/actions/actions.repository.ts`:
...
- [X] T008 [P] Add `title_pinned: boolean` to the frontend `Action` type in `frontend/src/api/types.ts`
...
- [X] T009 [US1] Widen repository `update()` in `backend/src/actions/actions.repository.ts` to
      persist `title` and `title_pinned` (add both to the `Pick<...>` type AND the `UPDATE action SET …` SQL)
- [X] T010 [US1] In `backend/src/actions/actions.service.ts` `update()`: accept optional `title`;
...
- [X] T011 [US1] Widen the `PATCH /api/actions/:id` body type in `backend/src/actions/actions.controller.ts`
      to accept `title?: string`, mapping the blank-title error to HTTP 400
- [X] T012 [US1] Add `title` to `updateAction` in `frontend/src/api/actions.ts`; add an inline title editor
...
- [X] T013 [US1] `backend/test/rename.test.ts` (node:test): rename sets the new title + `title_pinned=true`
...
- [X] T014 [US1] `frontend/…/ActionLine.test.tsx` (Vitest + Testing Library, `updateAction` mocked):
...
- [X] T016 [US2] Guard the title in the reconcile merge branch in `backend/src/sources/reconcile.service.ts`:
...
- [X] T017 [US2] `backend/test/rename.test.ts` (extend) with a stubbed Claude/Google edge: a renamed
...
- [X] T019 [P] `backend/test/comparator.test.ts`: the FR-021 urgency ordering (overdue → due-today → future →
      undated; then due asc, priority, created asc) and Uncategorised always last
- [X] T020 [P] `backend/test/destination.test.ts`: `resolveDestination` returns JIRA browse URL for a `CM-###`
      title, ESS URL for a leave action when configured (else source fallback), and the source URL otherwise
- [X] T021 [P] `backend/test/reconcile.test.ts`: candidates sharing a non-null dedup_key collapse to one; a
...
- [X] T022 [P] `backend/test/extraction-rules.test.ts`: cover the ownership rules as far as they are unit-
...
- [X] T023 [P] `frontend/…/CategoryBoard.test.tsx` (Vitest): renders category groups in order with the overdue
...
- [X] T025 [P] Add a CHANGELOG.md entry (Decision/Reasoning/Trade-off/Alternatives/Supersedes) recording inline
...
Implemented and covered by automated tests — both suites green:
- backend `npm test`: 23 tests (rename + reconcile guard, comparator, destination, reconcile
  dedup/suppress/merge/stale, extraction-parsing)
- frontend `npm test`: 5 tests (ActionLine rename/blank/escape, CategoryBoard render)
- backend `npm run build` and frontend `npm run build` both pass.

Pending manual verification (needs the running app / live Google re-sync — cannot be executed
here, not faked):
- **T015** — S1/S2/S6 are automated (T013/T014); the live-app click-through is the only part left.
- **T018** — S3/S4/S5 need a real sync of a source, which requires OAuth + the Claude CLI.
- **T024** — the suite half is done and green; the manual quickstart smoke is the pending half.
